...
const flow = [
  { step: "01", title: "Browse", body: "Open Home or Map and scan nearby offers." },
  { step: "02", title: "Save", body: "Heart the businesses and offers you want to revisit." },
  { step: "03", title: "Reserve", body: "Claim now to lock in inventory before it runs out." },
  { step: "04", title: "Redeem", body: "Show your code at checkout and complete in seconds." },
];
...
export default function ConsumersPage() {
  return (
    <WebsiteShell>
      <WebsiteScrollReveal>
        <section className="relative overflow-hidden rounded-3xl border border-[#dbe3db] bg-gradient-to-br from-[#eef7f3] via-[#f9fbf9] to-[#edf5ff] p-8 sm:p-10">
          <div className="website-orb-slow absolute -right-20 -top-20 h-56 w-56 rounded-full bg-[#1f6d68]/15 blur-3xl" />
          <div className="relative z-10 max-w-3xl">
            <p className="inline-flex items-center gap-2 rounded-full bg-white/85 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[#1f6d68]">
              <Sparkles className="h-3.5 w-3.5" />
              Consumer Experience
            </p>
            <h1 className="mt-4 text-4xl font-bold leading-tight text-[#1f2a2a] sm:text-5xl">
              Clean local discovery,
              <span className="text-[#1f6d68]"> from browse to redeem.</span>
            </h1>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/auth/signup" className="rounded-xl bg-[#1f6d68] px-4 py-2.5 text-sm font-semibold text-white">
                Start as consumer
              </Link>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            {flow.map((item) => (
              <article key={item.step} className="rounded-xl border border-[#e4e8e4] p-4">
                <p className="text-xs font-bold uppercase tracking-wider text-[#1f6d68]">Step {item.step}</p>
      ...
      <WebsiteScrollReveal delayMs={170} className="mt-8" variant="pop">
        <section className="rounded-2xl border border-[#1f6d68]/20 bg-[#1f6d68] p-6 text-white">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link href="/auth/signup" className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-[#1f6d68]">
              Free Download
